        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12 col-md-12 col-sm-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">All Trashed Area Code List</h3>
                        </div>
                        <!-- ./card-header -->
                        <div class="card-body">
                            <table class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    @foreach ($areas->where('deleted_at', '!=', Null) as $area)
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ $area->id }}</td>
                                        <td>{{ $area->area_name ?? 'NULL' }}</td>
                                        <td>{{ $area->country->name ?? 'NULL' }}</td>
                                        <td>
                                            {{ $area->area_code ?? 'NULL' }}
                                        </td>
                                        <td>
                                            <a href="javascript:void(0)" onclick="confirm('Do you want to restore?') || event.stopImmediatePropagation()" class="btn btn-xs bg-success" wire:click="restore({{$area->id}})" data-toggle="tooltip" data-placement="top" title="Restore">
