@section('title', 'Area Code Trashed List')
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <table class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    @foreach ($zips->where('deleted_at', '!=', Null) as $zip)
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ $zip->id }}</td>
                                        <td>{{ $zip->areas->area_name ?? 'NULL' }}</td>
                                        <td>
                                            {{ $zip->zip_code ?? 'NULL' }}
                                        </td>
                                        <td>
                                            <a href="javascript:void(0)" onclick="confirm('Do you want to restore?') || event.stopImmediatePropagation()" class="btn btn-xs bg-success" wire:click="restore({{$zip->id}})" data-toggle="tooltip" data-placement="top" title="Restore">
