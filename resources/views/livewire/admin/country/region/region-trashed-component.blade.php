@section('title', 'Regions Trashed List')
        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12 col-md-12 col-sm-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">All Trashed Region List</h3>
                        </div>
                        <!-- ./card-header -->
                        <div class="card-body">
                            <table class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    @foreach ($regions->where('deleted_at', '!=', Null) as $region)
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ $region->id }}</td>
                                        <td>{{ $region->name ?? 'NULL' }}</td>
                                        <td>{{ $region->country->name ?? 'NULL' }}</td>
                                        <td>
                                            {{ $region->code ?? 'NULL' }}
                                        </td>
                                        <td>
                                            <a href="javascript:void(0)" onclick="confirm('Do you want to restore?') || event.stopImmediatePropagation()" class="btn btn-xs bg-success" wire:click="restore({{$region->id}})" data-toggle="tooltip" data-placement="top" title="Restore">
